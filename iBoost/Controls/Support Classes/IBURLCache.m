...
#define IB_LARGE_IMAGE_SIZE (600*400)

static NSString* kDefaultCacheName = @"Three20";

static IBURLCache* gSharedCache = nil;
static NSMutableDictionary* gNamedCaches = nil;

//////////////////////////////////////////////////////////////////////////////////////////////////

@implementation IBURLCache
...
+ (IBURLCache*)cacheWithName:(NSString*)name {
  if (!gNamedCaches) {
    gNamedCaches = [[NSMutableDictionary alloc] init];
  }
  IBURLCache* cache = [gNamedCaches objectForKey:name];
  if (!cache) {
    cache = [[[IBURLCache alloc] initWithName:name] autorelease];
    [gNamedCaches setObject:cache forKey:name];
  }
  return cache;
}

+ (IBURLCache*)sharedCache {
  if (!gSharedCache) {
    gSharedCache = [[IBURLCache alloc] init];
  }
  return gSharedCache;
}

+ (void)setSharedCache:(IBURLCache*)cache {
...
- (void)storeImage:(UIImage*)image forURL:(NSString*)URL force:(BOOL)force {
  if (image && (force || !_disableImageCache)) {
    ...
    if (force || pixelCount < IB_LARGE_IMAGE_SIZE) {
...
- (id)initWithName:(NSString*)name {
  if (self == [super init]) {
    _name = [name copy];
    _cachePath = [[IBURLCache cachePathWithName:name] retain];
    _imageCache = nil;
...
- (void)dealloc {
  [[NSNotificationCenter defaultCenter] removeObserver:self
                                        name:UIApplicationDidReceiveMemoryWarningNotification  
                                        object:nil];  
  RELEASE_AND_NIL(_name);
  RELEASE_AND_NIL(_imageCache);
  RELEASE_AND_NIL(_imageSortedList);
  RELEASE_AND_NIL(_cachePath);
  [super dealloc];
}
...
- (NSData*)dataForURL:(NSString*)URL {
  return [self dataForURL:URL expires:IB_CACHE_EXPIRATION_AGE_NEVER timestamp:nil];
}
