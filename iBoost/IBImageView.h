...
@protocol IBImageViewDelegate;

/**
 * A view that asynchronously loads an image and subsequently displays it.
 */
@interface IBImageView : UIView <IBURLRequestDelegate> {
  IBURLRequest* _request;
  NSString*     _urlPath;
  UIImage*      _image;
  UIImage*      _defaultImage;
  BOOL          _autoresizesToImage;

  id<IBImageViewDelegate> _delegate;
}
 ...
 */
@property(nonatomic,assign)   id<IBImageViewDelegate> delegate;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
@protocol IBImageViewDelegate <NSObject>
@optional

/**
 * Called when the image begins loading asynchronously.
 */
- (void)imageViewDidStartLoad:(IBImageView*)imageView;

/**
 * Called when the image finishes loading asynchronously.
 */
- (void)imageView:(IBImageView*)imageView didLoadImage:(UIImage*)image;

/**
 * Called when the image failed to load asynchronously.
 * If error is nil then the request was cancelled.
 */
- (void)imageView:(IBImageView*)imageView didFailLoadWithError:(NSError*)error;
